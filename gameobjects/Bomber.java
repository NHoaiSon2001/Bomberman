...
public class Bomber extends Player {
   ...
   public Bomber(Float position, BufferedImage[][] spriteMap) {
   ...
   private void plantBomb() {
      ...
      float y = (float)(Math.round((this.position.getY() + 16.0D) / 32.0D) * 32L);
      Float spawnLocation = new Float(x, y);
   ...
   public void handleCollision(Bomb collidingObj) {
      ...
      if (intersection.getWidth() >= intersection.getHeight() && intersection.getHeight() <= 6.0D && Math.abs(this.collider.getCenterX() - collidingObj.collider.getCenterX()) <= 8.0D) {
      ...
      if (intersection.getHeight() >= intersection.getWidth() && intersection.getWidth() <= 6.0D && Math.abs(this.collider.getCenterY() - collidingObj.collider.getCenterY()) <= 8.0D) {
         if (this.kick && !collidingObj.isKicked()) {
